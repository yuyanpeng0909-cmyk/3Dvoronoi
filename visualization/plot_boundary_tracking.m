...
%   params: 参数结构体
    ...
    figure('Name', '动态边界追踪指标', 'Position', [100 100 1000 600]);
    ...
    h1 = plot(t, plume_extent / 1e6, 'Color', [0.85 0.33 0.1], 'LineWidth', 2.2, ...
              'DisplayName', '羽流有效体积');
    ylabel('羽流有效体积 (×10⁶ m³)', 'FontSize', 12);
    set(gca, 'YColor', [0.85 0.33 0.1]);

    % 右Y轴：距离/RMSE
    yyaxis right;
    hold on;
    h2 = plot(t, boundary_distances, 'b-', 'LineWidth', 2, 'DisplayName', 'Proposed 边界距离');
    ...
    xlabel('泄漏时间 (s)', 'FontSize', 12);
    title('溢油扩散范围与边界追踪性能', 'FontSize', 14);
