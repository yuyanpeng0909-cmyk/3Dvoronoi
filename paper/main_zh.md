...
图 3 展示覆盖质量对比。Standard CVT 最终覆盖代价较低，但其原因是 AUV 向浓度核心区聚集，牺牲了边界监测能力。

![覆盖质量对比](figures/step8_coverage_quality.png)

图 4 展示动态覆盖率和边界 RMSE 对比。
...
结果表明，所提 CVT-DBT 方法相比 Standard CVT 将边界 RMSE 由 66.75 m 降低至 15.94 m，降低约 76.1%。同时，动态覆盖率由 38.24% 提升至 58.86%。这说明动态边界追踪项和空间均衡分配机制能够有效缓解集群塌陷问题。

图 5 展示 AUV 速度曲线。所有 AUV 速度均满足 $v_{\max}=3.5$ m/s 约束，且速度变化平滑，说明控制输入具有工程可实现性。
...
仿真实验表明，Standard CVT 虽然获得较低覆盖代价，但存在明显集群塌陷，最终动态覆盖率仅为 38.24%，边界 RMSE 为 66.75 m。所提 CVT-DBT 方法在覆盖代价适度增加的前提下，将动态覆盖率提升至 58.86%，并将边界 RMSE 降低至 15.94 m，同时满足 AUV 速度约束。未来工作将进一步考虑 AUV 避碰、通信受限、复杂海底地形和控制障碍函数安全约束。
